    ...
    methods: {
        invite() {
            const popup = window.open(
                'about:blank',
                '',
                'toolbar=no,scrollbars=yes,resizable=yes,width=498,height=666'
            );
            fetch('https://botpyro-testing.herokuapp.com/api/invite')
                .then(res => res.json())
                .then(({ invite }) => {
                    popup.location = invite;
                });
        },
        login() {} // eslint-disable-line no-empty-function
    }
};
</script>
